    ...
    while (i < inText.length){  //прогоняет i через весь текст
        ...
        if(n>=4) {                                      //если n превзошёл 4, то его надо сжать
            if(n>=255){
                ...
                outcode+=("#" + String.fromCharCode(255) + symb).repeat(m)+ "#" + String.fromCharCode(n-m*255) + symb; //если счетчик превысил 255, то после решетки идёт зашифрованное 255 и действите повторяется столько раз, чему равен m + идёт зашифрованное число меньше 255, если оно есть
