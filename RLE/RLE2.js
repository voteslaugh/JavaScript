...
fs.readFile("outcode.txt", "utf-8", function(err, data){ //читает данные в файле ауткод, обращенные в формат utf-8
    ...
    while (i < inText.length){  //прогоняет i через весь текст
        ...
        if (symb=="#"){
            ...
            outcode+=inText.charAt(i+2).repeat(numbersymb+3); //берем символ, который находится в 2 знаках от решетки, и повторяем его столько раз, чему равен декодированный numbersymb
